...
show_meal(X) :- meal_chosen(X).
show_bread(X) :- bread_chosen(X).
show_meat(X) :- meat_chosen(X).
show_veggie(X) :- veggie_chosen(X).
show_sauce(X) :- sauce_chosen(X).
show_topup(X) :- topup_chosen(X).
show_side(X) :- side_chosen(X).